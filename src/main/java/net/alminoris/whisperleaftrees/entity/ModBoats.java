...
public class ModBoats
{
    ...
    {{
        for(String name : ModBlockSetsHelper.WOOD_NAMES)
            put(name, Identifier.of(net.alminoris.whisperleaftrees.WhisperleafTrees.MOD_ID, name+"_boat"));
    }};

    public static Dictionary<String, Identifier> chestBoatIDs = new Hashtable<>()
    {{
        for(String name : ModBlockSetsHelper.WOOD_NAMES)
            put(name, Identifier.of(net.alminoris.whisperleaftrees.WhisperleafTrees.MOD_ID, name+"_chest_boat"));
    }};
